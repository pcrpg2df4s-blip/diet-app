...
try {
    if (window.Telegram && window.Telegram.WebApp) {
        ...
        tg.ready();
        tg.expand();
        // Включаем черную тему для статус-бара, если нужно
...
// 3. ФУНКЦИЯ: ГЕНЕРАЦИЯ КАЛЕНДАРЯ (ПН, ВТ...)
function generateCalendar() {
    ...
    // Преобразуем JS день (0=ВС) в наш формат (0=ПН, 6=ВС)
    const todayIndex = today === 0 ? 6 : today - 1; 

    // Генерируем даты для текущей недели
    // Для простоты пока просто ставим числа, предполагая начало недели
    const curr = new Date(); 
    const first = curr.getDate() - curr.getDay() + 1; // Понедельник

    days.forEach((d, index) => {
        // Вычисляем число месяца для этого дня
        // Это упрощенная логика, в реальном приложении нужна полноценная библиотека дат
...
function updateUI() {
    ...
    // Рассчитываем цели БЖУ (примерно 30/30/40%)
    ...
    // Главный круг (Радиус 50)
    // Важно: в дизайне "полный круг" = цель выполнена. 
    // Значит заполняем circle пропорционально съеденному.
    setCircle('cal-progress-circle', consumed, goal, 50);

    // Маленькие круги (Радиус 20)
...
function renderFoodList(raw) {
    ...
    try {
        ...
        // Берем последние 4 записи (reverse + slice)
        list = list.reverse().slice(0, 4);

        if(list.length === 0) {
            con.innerHTML = '<p style="text-align:center;color:#ccc">Пока пусто</p>'; 
            return;
        }
        ...
        list.forEach(item => {
            ...
            // Время (фейковое или из данных, если есть)
            const time = "12:30 PM"; // Заглушка, т.к. в данных времени нет
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Навигация (если кнопки остались)
